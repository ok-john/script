...
sub get_date {
   ...
   $date = "$days[$wday] $mday $months[$mon] 19$year om $hour\:$min\:$sec";
   ...
   if ($CONFIG{'redirect'} =~ /http\:\/\/.*\..*/) {
	 ...
	 print "  <title>$CONFIG{'title'}</title>\n";
      ...
      if ($CONFIG{'title'}) {
         ...
         print "<h1>Dank u voor het invullen van dit formulier</h1>\n";
      }

      print "Beneden staat wat u heeft gestuurd naar $CONFIG{'recipient'} op ";
      print "$date.\n";
      print "U krijgt zo snel mogelijk antwoord.\n";
      print "<hr>\n";
      ...
      if ($CONFIG{'sort'} eq 'alphabetic') {
         ...
         foreach $sorted_field (@sorted_fields) {
            ...
            if ($FORM{$sorted_field}) {
               print "<b>$sorted_field:</b> $FORM{$sorted_field}<br>\n";
 	    }
         }
      }
      else {
      ...
      print "<P>[<A HREF=\"feedback.html\">Terug</A>]\n";
      print "</body>\n";
      print "</html>\n";
   }
}
...
sub error {
   ...
   if ($error eq 'bad_referer') {
      ...
      print "De volgende velden werden in het formulier leeg gelaten:<p>\n";
      ...
      print "<p><hr><p>\n";
      print "Deze velden moeten ingevuld worden voordat het bericht kan worden verstuurd.\n";
      print "Keer aub terug naar het <a href=\"$ENV{'HTTP_REFERER'}\">formulier</a> en probeer opnieuw.\n";
      print "</body></html>\n";
   }
   exit;
}
